...
function calcLabRating(reviews) {
    if (reviews.length === 0)
        return { overall: 0, cleanliness: 0, staffBehaviour: 0, waitTime: 0, totalReviews: 0 };
    const count = reviews.length;
    const t = reviews.reduce((acc, r) => {
...
function calcDefaultRating(reviews) {
    if (reviews.length === 0)
        return { overall: 0, cleanliness: 0, staffBehaviour: 0, waitTime: 0, totalReviews: 0 };
    const count = reviews.length;
    const t = reviews.reduce((acc, r) => {
...
function recalculate(type, reviews) {
...
async function main() {
    ...
    const fortis = await prisma.institution.create({
        data: {
            ...
            type: 'HOSPITAL',
            description: 'Fortis Hospital Noida is a multi-specialty tertiary care hospital offering world-class medical services. Located in Sector 62, it is equipped with state-of-the-art infrastructure and a team of highly experienced doctors.',
            city: 'Noida',
            pincode: '201301',
    ...
    const apollo = await prisma.institution.create({
        data: {
            ...
            description: 'Apollo Diagnostics is a leading diagnostic chain offering a comprehensive range of pathology and radiology services. Our Sector 18 centre is equipped with advanced imaging technology and a NABL-accredited lab.',
            city: 'Noida',
            pincode: '201301',
    ...
    const cloudnine = await prisma.institution.create({
        data: {
            ...
            type: 'NURSING_HOME',
            description: 'Cloudnine is a premium chain of maternity and childcare hospitals. Our Sector 45 facility specialises in obstetrics, gynaecology, and neonatology, providing a safe and comfortable environment for mothers and newborns.',
            city: 'Noida',
            pincode: '201303',
    ...
    const insertReviews = async (institutionId, reviewDataList) => {
        const created = [];
    ...
    const updateRatings = async (institutionId, type) => {
        const reviews = await prisma.review.findMany({
            where: { institutionId, status: client_1.ReviewStatus.APPROVED, isDeleted: false },
        });
        ...
        console.log(`  → ${type} ratings: overall=${ratings.overall}, cleanliness=${ratings.cleanliness}, staff=${ratings.staffBehaviour}, waitTime=${ratings.waitTime}`);
    };
    ...
    const firstFortisReview = await prisma.review.findFirst({
        where: { institutionId: fortis.id, status: client_1.ReviewStatus.APPROVED },
    ...
    const firstApolloReview = await prisma.review.findFirst({
        where: { institutionId: apollo.id, status: client_1.ReviewStatus.APPROVED },
        orderBy: { createdAt: 'asc' },
    });
    ...
    const firstCloudnineReview = await prisma.review.findFirst({
        where: { institutionId: cloudnine.id, status: client_1.ReviewStatus.APPROVED },
        orderBy: { createdAt: 'asc' },
    });
